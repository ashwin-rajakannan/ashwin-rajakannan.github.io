...
const ProjectCard: React.FC<ProjectCardProps> = ({
  ...
  function getShortDesc(desc: string, maxWords: number) {
    if (!desc) return "";
  ...
  return (
    <>
      <div
        className={cn(
          "h-full flex flex-col w-full overflow-hidden bg-slate-50 dark:bg-[#121417] border border-slate-200 dark:border-transparent hover:shadow-lg hover:shadow-[#06B6D4]/20 transition-all duration-300 ease-out justify-between",
          expanded ? "" : "h-[520px]",
          className
        )}
      >
            ...
            {!isPaperOnly ? (
              (isTextSummarization || isSentimentAnalysis) ? (
                ...
                >
                  ...
                  {image && !video && (
                    <Image
                      src={image?.startsWith('http') ? image : assetPrefix + image?.replace(/^\//, '')}
                      alt={title}
                      width={500}
                      height={300}
         ...
         <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 dark:bg-black/80 backdrop-blur-sm">
           <div className="relative w-full max-w-xs sm:max-w-sm md:max-w-md min-h-[380px] sm:min-h-[420px] flex flex-col justify-center items-center bg-[#121417] dark:bg-[#18181B] border-2 border-[#06B6D4] rounded-xl shadow-2xl p-8 animate-fadeIn">
             ...
             <h2 className="text-2xl font-bold text-white text-center mb-3">
               Try the Demo! <span className="ml-1">✨</span>
             </h2>
             {/* Description */}
             ...
             <div className="flex gap-4 w-full justify-center">
               <a
                 href={
                   isTextSummarization
                     ? "/projects/text-summarization-demo"
                     : "/projects/sentiment-analysis-demo"
                 }
